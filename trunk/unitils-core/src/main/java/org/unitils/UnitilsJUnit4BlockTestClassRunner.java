...
public class UnitilsJUnit4BlockTestClassRunner extends BlockJUnit4ClassRunner {
	...
	public UnitilsJUnit4BlockTestClassRunner(Class<?> testClass)
	...
	protected Statement classBlock(RunNotifier runNotifier) {
		...
		return new Statement() {
			...
			public void evaluate() throws Throwable {
				getTestListener()
						.beforeTestClass(getTestClass().getJavaClass());
	...
	@Override
	protected Statement methodInvoker(final FrameworkMethod frameworkMethod,
			final Object o) {
		return new InvokeMethod(frameworkMethod, o) {
			...
			public void evaluate() throws Throwable {
				...
				try {
					getTestListener().beforeTestMethod(o,
							frameworkMethod.getMethod());
					super.evaluate();
				} catch (Throwable t) {
					threw = t;
				} finally {
					getTestListener().afterTestMethod(o,
							frameworkMethod.getMethod(), threw);
				}
				if (threw != null) {
					throw threw;
				}
	...
	protected Statement withAfters(final FrameworkMethod method,
		...
		List<FrameworkMethod> afters = getTestClass().getAnnotatedMethods(
				After.class);
		return new RunAfters(statement, afters, target, method);
	}
	...
	private class RunAfters extends Statement {
		...
		public RunAfters(Statement next, List<FrameworkMethod> afters,
				Object target, FrameworkMethod method) {
			fNext = next;
			fAfters = afters;
			fTarget = target;
			fMethod = method;
		...
		public void evaluate() throws Throwable {
			...
			try {
				...
				getTestListener().afterTestTearDown(fTarget,
						fMethod.getMethod());
			}
