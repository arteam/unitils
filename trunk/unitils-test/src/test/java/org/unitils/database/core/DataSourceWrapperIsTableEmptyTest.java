 ...
 * @author Tim Ducheyne
 */
public class DataSourceWrapperIsTableEmptyTest extends UnitilsJUnit4 {
    ...
    public void tableEmpty() throws Exception {
        simpleJdbcTemplateMock.returns(0L).queryForObject("select count(1) from my_table", Long.class);

        boolean result = dataSourceWrapper.getMock().isTableEmpty("my_table");
        assertTrue(result);
    }

    @Test
    public void tableNotEmpty() throws Exception {
    ...
    public void exceptionWhenEmptyTableName() throws Exception {
        try {
            dataSourceWrapper.getMock().isTableEmpty("");
