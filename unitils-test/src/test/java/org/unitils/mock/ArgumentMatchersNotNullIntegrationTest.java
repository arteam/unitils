 ...
 * @author Tim Ducheyne
 */
public class ArgumentMatchersNotNullIntegrationTest extends UnitilsJUnit4 {
    ...
    public void matchWhenNotNull() {
        mockObject.returns("ok").method(notNull(String.class));

        String result = mockObject.getMock().method("value");
        assertEquals("ok", result);
        mockObject.assertInvoked().method(notNull(String.class));
    }
    ...
    public void noMatchWhenNull() {
        ...
        assertNull(result);
        mockObject.assertNotInvoked().method(notNull(String.class));
    }

    @Test(expected = NullPointerException.class)
    public void nullPointerForPrimitiveValue() {
        mockObject.returns("ok").intMethod(notNull(Integer.class));
    }
