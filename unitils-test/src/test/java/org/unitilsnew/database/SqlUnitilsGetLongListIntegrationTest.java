 ...
 * @author Tim Ducheyne
 */
public class SqlUnitilsGetLongListIntegrationTest {
    ...
    public void defaultDatabase() throws Exception {
        List<Long> result = SqlUnitils.getLongList("select value from my_table");
        assertLenientEquals(asList(111, 222), result);
    }

    @Test
    public void namedDatabase() throws Exception {
        List<Long> result = SqlUnitils.getLongList("select value from my_table", "database2");
    ...
    @Test
    public void exceptionWhenWhenNotALongValue() throws Exception {
        try {
            SqlUnitils.getLongList("select other from my_table where value = 111");
            fail("UnitilsException expected");
        } catch (UnitilsException e) {
            assertEquals("Unable to execute statement: 'select other from my_table where value = 111'. Reason:\n" +
                    "StatementCallback; bad SQL grammar [select other from my_table where value = 111]; nested exception is java.sql.SQLException: Wrong data type: type: VARCHAR (12) expected: BIGINT value: xxx", e.getMessage());
        }
    }

    @Test
    public void emptyListWhenNoResultsFound() throws Exception {
        List<Long> result = SqlUnitils.getLongList("select value from my_table where value = '999'");
    ...
    public void exceptionWhenDatabaseNameNotFound() throws Exception {
        try {
            SqlUnitils.getLongList("select value from my_table", "xxx");
            fail("UnitilsException expected");
        } catch (UnitilsException e) {
    ...
    public void defaultDatabaseWhenNullDatabaseName() throws Exception {
        List<Long> result = SqlUnitils.getLongList("select value from my_table", null);
        assertLenientEquals(asList(111, 222), result);
    }
